...
class DeckShowViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let navBar: UINavigationBar = UINavigationBar(
                                        frame: CGRect(x: 0, y: 10, width: self.view.frame.width, height: 75))
        self.view.addSubview(navBar)
        let navItem = UINavigationItem(title: "Decks")
        let doneItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.cancel,
                                       target: self, action: #selector(DeckShowViewController.dismissVC))
        navItem.rightBarButtonItem = doneItem
        navBar.setItems([navItem], animated: false)

        deckEditView = DeckEditView(
            frame: CGRect(x: 0, y: navBar.frame.height, width: self.view.frame.width, height: self.view.frame.height))
...
extension DeckShowViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if countActive >= 25 {
            ...
            let message =   """
                            Currently you have \(countActive)/25 cards
                            in the \(deck.name) deck. Please activate
                            more cards or select a different deck.
                            """

            let alert = UIAlertController(title: "Not Enough Cards!",
                                          message: message,
